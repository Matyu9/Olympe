...
def send_test_email(database):
    try:
        ...
        subject = "MESSAGE DE TEST " + database.query(Config.content).filter(Config.name == "MAIL_VERIFICATION_SUJET").first()[0]
        message = "MESSAGE DE TEST \n" + database.query(Config.content).filter(Config.name == "MAIL_VERIFICATION_CONTENU").first()[0]
